...
#define AQI_WIFI_SSID_CONTAINER_SIZE   736, 171
...
HSM_STATE_HANDLER_DECLARE(aqi_wifi_state_idle);
HSM_STATE_HANDLER_DECLARE(aqi_wifi_state_config);
HSM_STATE_HANDLER_DECLARE(aqi_wifi_state_scan);
HSM_STATE_HANDLER_DECLARE(aqi_wifi_state_connect);

static const state_t _aqi_wifi_states[] = {
    HSM_STATE_DECLARE(aqi_wifi_state_idle),
    HSM_STATE_DECLARE(aqi_wifi_state_config),
    HSM_STATE_DECLARE(aqi_wifi_state_scan),
    HSM_STATE_DECLARE(aqi_wifi_state_connect),
};
...
static int __aqi_wifi_create_ssid(aqi_wifi_t *aqi_wifi , uint16_t ap_number, wifi_ap_record_t ap_info[])
{
    if (ap_number == 0 || aqi_wifi->ssid.number)
        return ESP_FAIL;

    aqi_wifi->ssid.number = ap_number;
    aqi_wifi->ssid.container = aqi_selector_create_container(ui_ContainerSettingWifi,
    ...
    for (int i = 0; i < ap_number; i++)
    {
        ...
        aqi_wifi->ssid.ui[i].lock = aqi_selector_create_image(aqi_wifi->ssid.ui[i].selector->name,
...
{
    aqi_wifi_t *aqi_wifi = (aqi_wifi_t *)timer->user_data;
    ...
    if (abs(ap_info.rssi - aqi_wifi->last_rssi) >= 3 || aqi_wifi->last_rssi == 0)
    {
        ESP_LOGD(TAG, "WiFi signal strength: %d dBm", ap_info.rssi);

        aqi_wifi->last_rssi = ap_info.rssi;
...
static void __ui_event_handler(lv_event_t *event)
{
    ...
    {
        aqi_wifi_event(aqi_wifi_event_switch_click);
    }
    else if (target== ui_SwitchSettingTabWifiEnable)
    {
        aqi_wifi_event(aqi_wifi_event_tab_switch_click);
    }
...
int aqi_wifi_init()
{
    ...
    lv_obj_add_event_cb(ui_SwitchSettingTabWifiEnable, &__ui_event_handler, LV_EVENT_CLICKED, &_aqi_wifi);
...
/********************************** Idle State **********************************/
static state_machine_result_t __aqi_wifi_state_idle_entry_handler(state_machine_t* const pState)
{
    ESP_LOGI(TAG, "Entering to idle state");
    return EVENT_HANDLED;
}

static state_machine_result_t __aqi_wifi_state_idle_handler(state_machine_t* const pState)
{
    ...
    switch (pState->Event)
    {
        ...
        case aqi_wifi_event_tab_switch_click:
        {
            ESP_LOGI(TAG, "Wi-Fi enable switch tab toggled");

            // Sync state of ui_SwitchSettingWifiEnable with ui_SwitchSettingTabWifiEnable
            ...
            {
                lv_obj_clear_state(ui_SwitchSettingWifiEnable, LV_STATE_CHECKED);
                __aqi_wifi_disconnect(aqi_wifi);
            }
            break;
        }
...
static state_machine_result_t __aqi_wifi_state_idle_exit_handler(state_machine_t* const pState)
{
...
/********************************** Config State **********************************/
static state_machine_result_t __aqi_wifi_state_config_entry_handler(state_machine_t* const pState)
{
    ESP_LOGI(TAG, "Entering to config state");
    return EVENT_HANDLED;
}

static state_machine_result_t __aqi_wifi_state_config_handler(state_machine_t* const pState)
{
    aqi_wifi_t* const aqi_wifi = (aqi_wifi_t*)pState;

    switch (pState->Event)
    {
        case aqi_wifi_event_switch_click:
        {
            ESP_LOGI(TAG, "Wi-Fi enable switch toggled");

            // Sync state of ui_SwitchSettingTabWifiEnable with ui_SwitchSettingWifiEnable
            if (lv_obj_has_state(ui_SwitchSettingWifiEnable, LV_STATE_CHECKED))
            {
                lv_obj_add_state(ui_SwitchSettingTabWifiEnable, LV_STATE_CHECKED);
            }
            else
            {
                lv_obj_clear_state(ui_SwitchSettingTabWifiEnable, LV_STATE_CHECKED);
            }
...
static state_machine_result_t __aqi_wifi_state_config_exit_handler(state_machine_t* const pState)
{
...
/********************************** Scan State **********************************/
static state_machine_result_t __aqi_wifi_state_scan_entry_handler(state_machine_t* const pState)
{
    ...
    lv_obj_clear_flag(ui_ButtonSettingSelect, LV_OBJ_FLAG_CLICKABLE);
...
static state_machine_result_t __aqi_wifi_state_scan_handler(state_machine_t* const pState)
{
    aqi_wifi_t* const aqi_wifi = (aqi_wifi_t*)pState;
...
static state_machine_result_t __aqi_wifi_state_scan_exit_handler(state_machine_t* const pState)
{
    ...
    lv_obj_add_flag(ui_ButtonSettingSelect, LV_OBJ_FLAG_CLICKABLE);

    return EVENT_HANDLED;
}

/********************************** Connect State **********************************/
static state_machine_result_t __aqi_wifi_state_connect_entry_handler(state_machine_t* const pState)
{
...
static state_machine_result_t __aqi_wifi_state_connect_handler(state_machine_t* const pState)
{
    aqi_wifi_t* const aqi_wifi = (aqi_wifi_t*)pState;
...
static state_machine_result_t __aqi_wifi_state_connect_exit_handler(state_machine_t* const pState)
{
